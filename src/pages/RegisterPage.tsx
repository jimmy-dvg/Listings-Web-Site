...
export function RegisterPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage(error ? error.message : 'Registration link sent. Check your inbox.')
  }

  return (
    <section className="mx-auto max-w-md space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <h1 className="text-2xl font-semibold text-slate-900">Register</h1>
      <p className="text-sm text-slate-600">Create an account with your email via magic link.</p>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        >
          {isSubmitting ? 'Sending...' : 'Register with magic link'}
        </button>
      </form>
